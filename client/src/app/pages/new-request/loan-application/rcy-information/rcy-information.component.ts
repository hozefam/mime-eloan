...
export class LoanApplicationRCYInformationComponent implements OnInit {
  ...
  constructor(private communicationsService: CommunicationsService, private spinnerService: Ng4LoadingSpinnerService, private calendar: NgbCalendar, private loanApplicationService: LoanApplicationService,
    private customerProfileService: CustomerProfileService, private commonService: CommonService, private rcyInfoService: RCYInfoService, private toastr: ToastrService, private modalService: NgbModal, private router: Router
  ...
  uploadFile(event, type) {
    try {
      ...
      if (event.target.files && event.target.files.length > 0) {
        ...
        if (this.rcyInfoService.AttachmentExtentionTypes.indexOf(file.name.toLowerCase().substring(file.name.toLowerCase().lastIndexOf('.') + 1)) === -1) {
    ...
    if (
      ...
      try {
        ...
        this.material_object.forEach(element => {
          ...
          element.RCY_RequestId = this.model.id;
        ...
        this.model.RCY_Material = this.material_object;
        this.model.RCY_ProductServices = this.product_services_object;

        //this.timer = Observable.timer(5000); // 5000 millisecond means 5 seconds
        this.rcyInfoService.submitRequest(this.model).then(res => {
  ...
  onClickLoanApplicationTab(tab_number) {
    ...
    if (this.startedFilling == 0) {
      ...
      this.commonService.showFailureToast("Complete filling the rcy Information !");
