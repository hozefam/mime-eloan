...
import { ModalLandForm1ModalsComponent } from '../modon-land-form1/modon-landform1-modals/modon-landform1-modals.component';
...
@Component({
  selector: "land-form1",
  templateUrl: "./modon-land-logistic-form.component.html",
  styleUrls: ["./modon-land-logistic-form.component.scss"],
  providers: [
...
export class ModonLandLogisticFormComponent implements OnInit {
...
LRequestReason=[];
  ...
  LProducts=[];
  LProductsFiltered=[]
  LProductSubFamilyId=[{Code:37,title:"   وحدات جاهزة  ",titleEn:"Ready units "},{Code:42,title:" اراضي    ",titleEn:"Lands    "}];
  LMeasuringUnit=[];
...
RawMaterialsDetails=[];
  ...
  resolveGetModonLandForm1(res){
    ...
    this.spinnerService.hide();
    this.GetProductsIds(res.productSubFamilyId);
    this.input ={
      Id: res.Id,
      ...
      "productSubFamilyId":res.productSubFamilyId,
 ...
 GetProductsIds(event){
   console.log(event);
   this.LProductsFiltered=this.LProducts.filter((o)=>o.ProductId==event);
 }
  resolvegetMODONLookUP(res) {
    ...
    this.LProduct_Attachments=res.filter((o)=>o.LookUpType=="Product_Attachments_NotIndustrial");
    
    this.LProducts=res.filter((o)=>o.LookUpType=="Product_Type");
    ...
    this.LIndustrial_Cities_Factory=res.filter((o)=>o.LookUpType=="Industrial_Cities_Logistics");
    this.LIndustrial_Cities_Land=res.filter((o)=>o.LookUpType=="Industrial_Cities_Logistics");
        ...
        buttons: [
          {
            ...
            handler: (modal_data) => {
              ...
              this.form5Service.SubmitModon_ImportingDetails(post_data)
                .then((res) => (
                  this.onDataPost(res, importing_source_data, modal_data, "edit", "", projectOwnershipModal)
