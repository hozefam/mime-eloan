...
export class InputsTableComponent implements OnInit {
  ...
  //   "ProductCode": "w"
  // }]; 
  @Output() deletedElemrnt = new EventEmitter<any>(); 
  @Output() createdElemrnt = new EventEmitter<any>();
  @Output() updatedElemrnt = new EventEmitter<any>();    
  rows = [];
 //  rowStructueCells :Cell[]=[];  
  ...
  onEdit(index  ){ 
       this.rows[index].forEach(c=> {  c.disabled = false ; c.value='' ; c.editMode = true; }  ); 
       this.rowsOnEdit.push(index); 
   }
   onSave(index){ 
    this.updatedElemrnt.emit(this.getTheTargetedJson(index)); 
    this.rows[index].forEach(c=> {  c.disabled = true ;    c.editMode = false; }  );
    this.rowsOnEdit= this.rowsOnEdit.filter(e=> { return e!== index})
   ...
   onAdd(index){ 
    this.createdElemrnt.emit(this.getTheTargetedJson(index));  
    this.rows[index].forEach(c=> {  c.disabled = true ;    c.editMode = false; }  ); 
    this.rowsOnEdit = this.rowsOnEdit.map(e=> { return e+1 ; })
    let newRowStructure = _.cloneDeep( this.rowStructureCells);
    newRowStructure.forEach(c=>  { c.value =''; c.disabled =false; } )
    this.rows.unshift(newRowStructure);  
    /// emmit the holl value to be saved again ,  
  }
  onDelete(index){ 
    this.deletedElemrnt.emit(this.getTheTargetedJson(index)); 
...
export class Cell{ 
 ...
 //  key:Boolean ; 
  public value:string= '' ; 
  constructor ( public name:string, public tag: string ='input',   public type: string , public list:CellList[]  ,public key:Boolean  = false ,    public requierd:string=  "requierd" ,  editMode:Boolean = false      ){ 
    if (!this.value )
     this.disabled= false ; 
  }
}
